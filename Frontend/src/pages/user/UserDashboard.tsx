...
export default function UserDashboard() {
  ...
  const myReservations = useMemo(() => {
    ...
    return reservationsData.items.map((r: any) => {
      ...
      return {
        ...
        sport: court?.sport || 'Esporte',
        location: court ? `${court.city} • ${court.neighborhood}` : 'Local não disponível',
        date: start.toLocaleDateString('pt-BR', { weekday: 'long', year: 'numeric', month: '2-digit', day: '2-digit', timeZone: 'UTC' }),
  ...
  const dynamicAchievements = useMemo(() => {
    ...
    // Verifica se existem 10 reservas no mês atual para conquista
    ...
    return [
      { icon: Trophy,       label: 'Primeiro Jogo',    desc: '1ª reserva realizada',      earned: myReservations.length > 0 },
      { icon: Zap,          label: 'Frequentador',     desc: '5 reservas confirmadas',     earned: completedOrConfirmed >= 5 },
      { icon: Star,         label: 'Avaliador',         desc: '3 avaliações enviadas',      earned: Object.keys(ratings).length >= 3 },
      { icon: CalendarCheck, label: 'Dedicado',         desc: '10 reservas no mês',         earned: thisMonthRes >= 10 },
    ];
  }, [myReservations, reservationsData, ratings]);
    ...
    <div className="pt-28 pb-20 px-4 sm:px-6 max-w-5xl mx-auto space-y-10">
      ...
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
          Olá, <span className="font-bold text-gray-700 dark:text-gray-300">{firstName}</span> — acompanhe e gerencie suas reservas de quadras.
        </p>
      </div>
      ...
      {nextReservation && (
        <div className="relative overflow-hidden rounded-2xl bg-gray-900 dark:bg-background border border-white/10 shadow-2xl">
          <div
            className="absolute inset-0 bg-cover bg-center opacity-25 transition-transform duration-[10s] hover:scale-105"
            style={{ backgroundImage: `url("${nextReservation.img}")` }}
          />
          ...
          <div className="relative z-10 p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div className="flex flex-row sm:flex-col gap-2 shrink-0">
              ...
              >
                Ver Quadra <ArrowUpRight className="w-4 h-4" />
              </Link>
      ...
      <div className="bg-white dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.06] rounded-2xl overflow-hidden">
        ...
        {filteredReservations.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            ...
            <p className="text-sm font-bold text-gray-500 dark:text-gray-400">Nenhuma reserva encontrada</p>
            <p className="text-xs text-gray-400 mt-1">Tente mudar o filtro ou buscar outro termo</p>
            <Link to="/catalog" className="mt-5 text-xs font-black text-[#6aad00] dark:text-[#8CE600] flex items-center gap-1 hover:opacity-70 transition-opacity">
              Encontrar uma quadra <ArrowUpRight className="w-3.5 h-3.5" />
            </Link>
          </div>
        )}
                  ...
                  <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                    {r.status === 'completed' ? (
                      <div className="flex items-center gap-2">
                        ...
                        {!currentRating && (
                          <span className="text-[11px] text-gray-400">Avalie esta quadra</span>
                        )}
                      </div>
                    ) : r.status === 'confirmed' ? (
                    ...
                    >
                      Ver quadra <ChevronRight className="w-3.5 h-3.5" />
                    </Link>
                  </div>
                </div>
              </div>
        ...
        {filteredReservations.length > 0 && (
          <div className="px-5 py-4 border-t border-gray-100 dark:border-white/[0.06] flex items-center justify-between">
            ...
            <Link to="/catalog"
              ...
              Fazer nova reserva <ArrowUpRight className="w-3 h-3" />
            </Link>
          </div>
        )}
      </div>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#8CE600] to-[#5c9200] p-8 text-center shadow-2xl shadow-[#8CE600]/20">
        ...
        <div className="relative z-10">
          ...
          <h3 className="text-xl font-black text-gray-950 mb-2">Explore mais quadras</h3>
          <p className="text-sm text-gray-950/60 mb-6 max-w-md mx-auto">
            Mais de 50 quadras premium em todo o Brasil esperando pela sua próxima partida.
          </p>
          <Link
            to="/catalog"
